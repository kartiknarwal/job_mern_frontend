...
const MyJobs = () => {
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      try {
        const { data } = await axios.get(`${API_BASE_URL}/api/v1/job/getmyjobs`, {
    ...
    fetchJobs();
  }, [isAuthorized, user, navigateTo, API_BASE_URL]);
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      const { data } = await axios.delete(`${API_BASE_URL}/api/v1/job/delete/${jobId}`, {
        withCredentials: true,
      });
      toast.success(data.message);
